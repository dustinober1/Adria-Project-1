"""
Main FastAPI application - Production Optimized Version with HTTPS Support
"""
...
from backend.database_production import engine, get_db, Base
...
frontend_urls = os.getenv("FRONTEND_URL", "https://localhost:3000").split(",")
frontend_urls_dev = os.getenv("FRONTEND_URL_DEV", "http://localhost:3000,http://localhost:8000").split(",")
all_origins = [url.strip() for url in frontend_urls + frontend_urls_dev if url.strip()]
...
async def register(user_data: UserRegister, db: Session = Depends(get_db)):
    ...
    json_response.set_cookie(
        ...
        value=token,
        httponly=True,
        secure=secure_cookies,  # Set to True in production with HTTPS
...
async def forgot_password(request: ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    # In a real app, send email via an email service (like SendGrid, Mailgun, etc.)
    ...
    return {
        ...
        # In production, don't return the link in the response - send via email
    }
...
async def get_current_user_endpoint(current_user: User = Depends(get_current_user)):
    ...
    logger.info(f"Current user info requested: {current_user.id}")
    return {
        "success": True,
        "message": "User found",
...
async def get_articles(db: Session = Depends(get_db)):
    ...
    logger.info("Public articles requested")
...
async def get_article_by_slug(slug: str, db: Session = Depends(get_db)):
    """Get article by slug"""
    logger.info(f"Article requested by slug: {slug}")
...
async def get_admin_stats(admin: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    ...
    logger.info(f"Admin stats requested by user: {admin.id}")
    return AdminStats(
...
async def get_users(admin: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    """Get all users"""
    logger.info(f"All users requested by admin: {admin.id}")
...
async def get_user_details(
    ...
    """Get user details"""
    logger.info(f"User details requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
...
async def delete_user(
    ...
    """Delete a user"""
    logger.info(f"User deletion requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def promote_user(
    ...
    logger.info(f"User promotion requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def demote_user(
    ...
    logger.info(f"User demotion requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def update_user_tier(
    ...
    """Update user tier"""
    logger.info(f"User tier update requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    db.commit()
    logger.info(f"User tier updated: {user_id}")
...
async def update_user_status(
    ...
    """Update user status"""
    logger.info(f"User status update requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    db.commit()
    logger.info(f"User status updated: {user_id}")
...
async def update_user_notes(
    ...
    logger.info(f"User notes update requested by admin {admin.id} for user: {user_id}")
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    db.commit()
    logger.info(f"User notes updated: {user_id}")
...
async def get_admin_articles(admin: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    ...
    logger.info(f"All articles requested by admin: {admin.id}")
...
async def create_article(
    ...
    """Create new article"""
    logger.info(f"New article creation requested by admin: {admin.id}")
    ...
    logger.info(f"New article created: {new_article.id}")
    return {
        "success": True,
...
async def update_article(
    ...
    """Update article"""
    logger.info(f"Article update requested by admin {admin.id} for article: {article_id}")
...
async def delete_article(
    ...
    """Delete article"""
    logger.info(f"Article deletion requested by admin {admin.id} for article: {article_id}")
    article = db.query(BlogArticle).filter(BlogArticle.id == article_id).first()
    if not article:
        raise HTTPException(status_code=404, detail="Article not found")
...
async def subscribe_email(email_data: EmailSubscribe, db: Session = Depends(get_db)):
    ...
    logger.info(f"Email subscription requested: {email_data.email}")
    ...
    logger.info(f"New subscriber added: {email_data.email}")
